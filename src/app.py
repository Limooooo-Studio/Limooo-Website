...
STATUS_PROVIDER_SUMMARIES = {
    "claude": (
        "/opt/claude-webhook/summary.json",
        "https://status.claude.com/api/v2/summary.json",
    ),
    "cloudflare": (
        "/opt/cloudflare-webhook/summary.json",
        "https://www.cloudflarestatus.com/api/v2/summary.json",
    ),
}
CLAUDE_SUMMARY_TTL_SECONDS = 60
_provider_summary_cache: dict[str, tuple[float, dict[str, object]]] = {}
_provider_summary_lock = threading.Lock()


def _provider_summary(provider: str) -> dict[str, object]:
    """Read the minute-by-minute local snapshot, then fall back to its public API."""
    now = time.monotonic()
    cached = _provider_summary_cache.get(provider)
    if cached and now - cached[0] < CLAUDE_SUMMARY_TTL_SECONDS:
        return cached[1]

    # Keep concurrent page refreshes from stampeding the upstream status API.
    with _provider_summary_lock:
        now = time.monotonic()
        cached = _provider_summary_cache.get(provider)
        if cached and now - cached[0] < CLAUDE_SUMMARY_TTL_SECONDS:
            return cached[1]
        snapshot_path, summary_url = STATUS_PROVIDER_SUMMARIES[provider]
        try:
        ...
        if not isinstance(payload, dict):
            raise ValueError(f"{provider} status summary is not a JSON object")
        _provider_summary_cache[provider] = (time.monotonic(), payload)
        return payload
...
def provider_status_page():
    ...
    source_updated = None
    try:
        payload = _provider_summary(provider_key)
        components = payload.get("components", [])
        incidents = payload.get("incidents", [])
        source_updated = payload.get("page", {}).get("updated_at")
    except Exception as exc:  # noqa: BLE001
        app.logger.warning("[%s-status] summary failed: %s", provider_key, exc)
        if provider_key == "cloudflare":
            # Cloudflare webhooks remain a useful degraded-mode source.
            incidents = [
                {
