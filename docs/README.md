...
| Deployment | systemd + rsync (`deploy.sh`) |
...
│   ├── geoip.py           # GeoIP maintenance: daily geo pre-cache + weekly GeoLite2 update (subcommands: cache/update)
│   └── build.py           # Pages 静态化构建（python3 src/build.py）
├── README.md              # this file
├── LICENSE
├── data/                  # runtime data (generated; git-ignored)
...
Injected via `keys/webauthn.env` (systemd `EnvironmentFile`), not committed to Git:
...
`keys/flask_secret.key` and `keys/appleid_encryption.key` are auto-generated by the app and migrated to `/etc/limooo/` (mode 600) at deploy time. Resolution order: environment variable > `/etc/limooo/` > project directory.

## Cron jobs

`deploy.sh` sets up the following crontab entries after deployment:

| Time | Job |
| --- | --- |
| Daily 02:00 | `src/geoip.py cache` pre-caches visitor geolocation |
| Daily 03:00 | `src/auto_block.py` scans logs and auto-bans malicious IPs |
| Wed 03:00 | `src/geoip.py update` updates the GeoLite2 databases |

## Deployment

To switch servers, just change `REMOTE_HOST` at the top of `deploy.sh`, then:

```bash
./deploy.sh
```

The script: rsyncs the code → installs missing dependencies (nginx/rsync/python3/ipset/docker) → migrates keys → restarts the systemd service → deploys the Nginx config (incl. HTTP/3) → downloads GeoLite2 → writes the crontab.
...
6. **DNS 已切换**：`limooo.cn` / `www` / `services` / `contact` / `auth` → CNAME `limooo.pages.dev`（proxied），自定义域名全部 active；`identity` / `visitor` / `appleid` / `redirect` / `xmpp` 保持服务器 A 记录，`*.limooo.cn` 通配 A 保留作为兜底
